-- =============================================================================
-- match_scholarships + match_score_breakdown — provider_trust boost
-- =============================================================================
-- Adds a small bounded ranking boost based on the linked provider's
-- trust_tier (added in 20260509010000). Compounds with the confidence
-- adjustment + completeness boost + engagement boost from prior trust-
-- calibration migrations: a row from a verified government program with
-- thorough extraction + saves traction surfaces above an unknown
-- provider's row at equal embedding similarity.
--
-- Bounded magnitude (max +0.020) so similarity stays dominant. Tier
-- mapping:
--   * 'high'    →  +0.020   (verified government / major foundation)
--   * 'medium'  →  +0.010
--   * 'low'     →  -0.005   (small penalty — known thin signal)
--   * 'unknown' →   0.000   (default; no effect)
--   * NULL      →   0.000   (no provider link; same as unknown)
--
-- Why not bigger: provider trust is correlated with confidence + the
-- existing similarity signal (well-known providers tend to have
-- comprehensive pages). We don't want to double-count.
-- =============================================================================

-- ─── match_scholarships (8-arg canonical) ─────────────────────────────
...
AS $$
  ...
  scored AS (
    SELECT
      ...
      c.similarity,
      -- Deadline boost (max +0.040 within 60 days)
      (CASE
      ...
      -- Value boost (logarithmic, max +0.030 at $100k+)
      (CASE
      ...
      -- Recency boost (max +0.020 if verified within 90 days)
      (CASE
      ...
      -- Confidence adjustment (max -0.040 at conf=0)
      (CASE
      ...
      -- Completeness boost (max +0.030 at score=18)
      (CASE
      ...
      -- Engagement boost (max +0.020 from saves last 30d)
      (
      ...
      -- Provider-trust boost (max +0.020 for 'high', +0.010 for 'medium', -0.005 for 'low')
      (
        SELECT
          CASE p.trust_tier
...
COMMENT ON FUNCTION public.match_scholarships(vector(1536), text, numeric, numeric, text, int, numeric, numeric) IS
  'Composite-ranked vector match. Score = similarity + deadline + value + recency + confidence_adj + completeness + engagement + provider_trust − country_crowding.';

-- ─── match_score_breakdown (UI explainer) ─────────────────────────────
...
AS $$
  ...
  SELECT
    ...
    -- Composite (matches ranking math, country-crowding penalty applied at top level so we don't include here)
    (r.similarity
     + (CASE
         WHEN r.application_deadline IS NULL THEN 0.0
         WHEN r.application_deadline < current_date THEN -0.020
...
COMMENT ON FUNCTION public.match_score_breakdown(uuid, vector(1536), text, numeric, numeric, text) IS
  'Per-row score explainer used by the UI popover. Mirrors match_scholarships composite math, plus engagement and provider_trust signals.';
