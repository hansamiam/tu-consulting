...
export const CompareDrawer = ({ open, onClose, universities, onRemove, language }: CompareDrawerProps) => {
  ...
  const rows: RowDef[] = [
    ...
    { label: l.visa, render: (u) => { const v = getAvgVisa(u); const labels = ["Easy", "Moderate", "Hard", "Hard", "Hard"]; return v != null ? <Badge variant="outline" className={`text-xs ${v <= 2 ? "border-green-500 text-green-600" : "border-destructive text-destructive"}`}>{labels[v - 1]}</Badge> : l.na; } },
    { label: l.programs, render: (u) => <span className="font-semibold">{u.programs?.length || 0}</span> },
    { label: l.scholarships, render: (u) => <span className="font-semibold">{u.scholarships?.length || 0}</span> },
